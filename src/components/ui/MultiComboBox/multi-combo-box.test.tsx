...
describe('MultiComboBox', () => {
    ...
    it('shows placeholder when nothing is selected', () => {
        render(<MultiComboBox options={OPTIONS} selected={[]} placeholder='Pick fruits' />);
        expect(screen.getByText('Pick fruits')).toBeInTheDocument();
    });

    it('shows "N selected" when items are selected', () => {
        render(<MultiComboBox options={OPTIONS} selected={['apple', 'banana']} />);
        expect(screen.getByText('2 selected')).toBeInTheDocument();
    });

    it('renders label when provided', () => {
        render(<MultiComboBox options={OPTIONS} selected={[]} label='Fruits' />);
    ...
    it('calls onChange with added value when an option is selected', async () => {
        ...
        render(<MultiComboBox options={OPTIONS} selected={[]} onChange={onChange} />);
        await user.click(screen.getByRole('combobox'));
    ...
    it('calls onChange with value removed when an already-selected option is clicked', async () => {
        const user = userEvent.setup();
        const onChange = vi.fn();
        render(<MultiComboBox options={OPTIONS} selected={['apple']} onChange={onChange} />);
        await user.click(screen.getByRole('combobox'));
        const appleOption = screen.getAllByText('Apple').find(
            (el) => el.closest('[data-command-item]'),
        );
    ...
    it('renders error message when error is provided', () => {
        render(<MultiComboBox options={OPTIONS} selected={[]} error='Selection required' />);
        expect(screen.getByText('Selection required')).toBeInTheDocument();
    });

    it('sets aria-invalid on trigger when error is provided', () => {
        render(<MultiComboBox options={OPTIONS} selected={[]} error='Invalid' />);
    ...
    it('shows notFoundText when search has no matches', async () => {
        const user = userEvent.setup();
        render(<MultiComboBox options={OPTIONS} selected={[]} notFoundText='Nothing here' />);
        await user.click(screen.getByRole('combobox'));
